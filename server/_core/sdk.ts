...
class SDKServer {
  ...
  async signSession(
    ...
    return new SignJWT({
      ...
      appId: payload.appId || "savanna-local",
      name: payload.name,
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (session.openId.startsWith(LOCAL_OPEN_ID_PREFIX)) {
      ...
      return user ?? buildLocalUser(session, signedInAt);
    }
...
function buildLocalUser(
  session: { openId: string; name: string; email?: string | null },
  now: Date
): AuthenticatedUser {
  const idSeed = createHash("sha256").update(session.openId).digest("hex").slice(0, 8);
  return {
    id: parseInt(idSeed, 16),
    openId: session.openId,
    name: session.name,
    email: session.email ?? null,
    loginMethod: "local",
