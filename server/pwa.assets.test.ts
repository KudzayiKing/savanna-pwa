...
describe("Savanna PWA assets", () => {
  ...
  it("keeps the SAVANNA Ramabhadra wordmark in the supplied Ink and Warm White colors", async () => {
    ...
    expect(styles).toContain("--ivory: #F7F1E3;");
    ...
    expect(styles).toContain("--bright-gold: #F2C14E;");
    expect(styles).toContain("--deep-gold: #A87820;");
    expect(styles).toContain("--success: #4CAF68;");
    expect(styles).toContain("--processing: #E5A72E;");
    expect(styles).toContain("--info: #5A9BD5;");
    expect(styles).toContain("--error: #C95C55;");
    ...
    expect(styles).toContain('[class~="bg-[#24482f]"] { background-color: #A87820 !important; }');
    expect(styles).toContain('[class~="hover:bg-[#1b3b25]"]:hover { background-color: #8C631B !important; }');
    expect(styles).toContain('[class~="text-[#31583a]"] { color: #A87820 !important; }');
    ...
    expect(styles).toContain('[class~="text-[#405340]"] { color: #6F6A60 !important; }');
    expect(styles).toContain('[class~="text-[#496348]"] { color: #A87820 !important; }');
  ...
  it("provides a mobile Stories header and a familiar mobile chat-list hierarchy", async () => {
    ...
    expect(shell).toContain('import { MobileNavIcon, type MobileNavIconName } from "@/components/AnimatedNavIcons"');
    ...
    expect(stories).toContain("<header className=\"savanna-mobile-header fixed inset-x-0 top-0 z-40");
    ...
    expect(stories).toContain('AnimatedMenuIcon size={26} pulse={menuPulse}');
    ...
    expect(stories).toContain('text-[#9a8467] dark:text-[#9AA1A6]">Your Story</span>');
    expect(stories).toContain('text-[#9a8467] dark:text-[#9AA1A6]">{group.authorName.split(" ")[0]}</span>');
    expect(stories).toContain("groupedStories.slice(0, 3).map");
    expect(stories).toContain("grid size-7 shrink-0 place-items-center rounded-full border-2 border-[#D9A441]");
    ...
    expect(messages).toContain('text-[#9a8467] dark:text-[#9AA1A6]');
    expect(stories).toContain('onPointerDown={() => setSearchPulse(current => current + 1)}');
    expect(stories).toContain('AnimatedSearchIcon size={26} pulse={searchPulse}');
    ...
    expect(shell).toContain('active ? "text-[#A87820] dark:text-[#D9A441]"');
    expect(shell).toContain('savanna-mobile-bottom-nav fixed inset-x-0 bottom-0 z-50');
    expect(shell).toContain('active ? "w-14 rounded-full bg-[#D9A441]/20 text-[#A87820] dark:text-[#D9A441]"');
    ...
    expect(shops).toContain("AnimatedSearchIcon size={16}");
    expect(shops).toContain('const SHOPPING_BANNER_URL = "/manus-storage/shopping_banner_89ed7fb7.png"');
    ...
    expect(learn).toContain("AnimatedSearchIcon size={16}");
    expect(learn).toContain('const LEARN_BANNER_URL = "/manus-storage/learn_banner_a6c3481b.png"');
    ...
    expect(animatedIcons).toContain('className="inline-flex -rotate-45 items-center justify-center"');
    expect(animatedIcons).toContain('onPointerEnter={play} onPointerDown={play}');
