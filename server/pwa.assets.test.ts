...
describe("Savanna PWA assets", () => {
  ...
  it("provides a mobile Stories header and a familiar mobile chat-list hierarchy", async () => {
    ...
    expect(stories).toContain("const prefersReducedMotion = useReducedMotion();");
    expect(stories).toContain("const storiesCollapseSpring: Transition");
    expect(stories).toContain("const [collapsedClusterWidth, setCollapsedClusterWidth] = useState(0);");
    expect(stories).toContain("const [centeredWordmarkX, setCenteredWordmarkX] = useState(0);");
    expect(stories).toContain("const collapsedClusterRef = useRef<HTMLDivElement>(null);");
    expect(stories).toContain("const mobileHeaderBarRef = useRef<HTMLDivElement>(null);");
    expect(stories).toContain("const mobileWordmarkRef = useRef<HTMLDivElement>(null);");
    expect(stories).toContain("new ResizeObserver(measure)");
    ...
    expect(stories).not.toContain('compact ? "hidden" : "block"');
    expect(stories).toContain("<motion.section");
    expect(stories).toContain("height: compact ? 0 : expandedHeight");
    expect(stories).toContain("opacity: compact ? 0 : 1");
    ...
    expect(stories).toContain('ref={mobileHeaderBarRef} className="relative h-[68px] px-4"');
    expect(stories).toContain('className="absolute inset-y-0 left-4 z-10 flex items-center"');
    expect(stories).toContain('className="savanna-mobile-wordmark-shell absolute inset-y-0 left-4 z-20 flex items-center"');
    expect(stories).toContain("x: compact ? centeredWordmarkX : 0");
    expect(stories).toContain("scale: compact ? 1.03 : 1");
    expect(stories).toContain("setCenteredWordmarkX(Math.max(0, (barWidth - wordmarkWidth) / 2 - 16));");
    ...
    expect(stories).toContain('className="savanna-collapsed-story-shell flex shrink-0 items-center overflow-hidden"');
    ...
    expect(styles).toContain(".savanna-app .savanna-collapsed-story-shell");
    expect(styles).toContain("margin-right: 0.75rem;");
