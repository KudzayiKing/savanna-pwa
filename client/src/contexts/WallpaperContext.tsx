...
export function WallpaperProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (setting.kind === "savanna-mobile" || setting.kind === "savanna-web") {
      ...
      root.style.setProperty("--savanna-wallpaper-image-portrait", `url("${theme === "dark" ? SAVANNA_WALLPAPERS[0].darkImage : SAVANNA_WALLPAPERS[0].lightImage}")`);
      root.style.setProperty("--savanna-wallpaper-image-landscape", `url("${theme === "dark" ? SAVANNA_WALLPAPERS[1].darkImage : SAVANNA_WALLPAPERS[1].lightImage}")`);
      image = "var(--savanna-wallpaper-image-portrait)";
    } else {
      root.style.removeProperty("--savanna-wallpaper-image-portrait");
      root.style.removeProperty("--savanna-wallpaper-image-landscape");
  ...
  }, [setting, theme, setColor, setSavannaWallpaper, setCustomImage, resetWallpaper]);
