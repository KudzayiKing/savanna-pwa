...
import { Ban, Check, EyeOff, KeyRound, Loader2, MessageCircle, Moon, ShieldCheck, Smartphone, Store, Sun, UserRound } from "lucide-react";
      ...
      <div className="savanna-profile-page mx-auto max-w-[960px] space-y-6">
        ...
        <section className="grid gap-4 md:grid-cols-4">
          {[
            ...
            { href: "/messages", label: "Groups", icon: MessageCircle },
            { href: "/messages", label: "Stories", icon: KeyRound },
            { href: "/shops/manage", label: "Business page", icon: Store },
          ].map(item => {
            const Icon = item.icon;
            return (
              <Link key={item.label} href={item.href} className="rounded-[24px] border border-[#eadfca] bg-white p-4 shadow-[0_10px_24px_rgba(94,58,11,0.035)]">
                <span className="grid size-11 place-items-center rounded-2xl bg-[#D9A441]/20 text-[#D9A441]"><Icon className="size-5" /></span>
                <p className="mt-4 text-sm font-semibold text-[#151A17]">{item.label}</p>
              </Link>
            );
          })}
        </section>

        <section className="flex items-center justify-between gap-4 rounded-[24px] border border-[#eadfca] bg-white p-5 shadow-[0_10px_24px_rgba(94,58,11,0.035)] dark:bg-[#202C33]">
        ...
        {isLoading ? <div className="grid min-h-64 place-items-center rounded-[26px] bg-white"><Loader2 className="size-5 animate-spin text-[#D9A441]" /></div> : <>
          <section className="rounded-[28px] border border-[#eadfca] bg-white p-6 shadow-[0_14px_35px_rgba(94,58,11,0.04)] sm:p-8">
