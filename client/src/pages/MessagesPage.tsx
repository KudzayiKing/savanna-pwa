...
function DesktopStoryRail({ items, onCreateStory }: { items: Array<{ id: number; label: string }>; onCreateStory: () => void }) {
  return <div className="savanna-desktop-story-rail" aria-label="Stories"><div className="flex gap-3 overflow-x-auto pb-1"><button type="button" onClick={onCreateStory} className="flex w-11 shrink-0 flex-col items-center gap-1 text-left"><span aria-label="Add to your Story" className="savanna-brand-token grid size-11 place-items-center rounded-full text-xs font-semibold"><AnimatedPlusIcon size={18} /></span><span className="max-w-11 truncate text-center text-[10px] text-[#5f6861]">Your</span></button>{items.map(item => <div key={item.id} className="flex w-11 shrink-0 flex-col items-center gap-1"><span aria-label={`${item.label}'s Story`} className="savanna-brand-token grid size-11 place-items-center rounded-full text-xs font-semibold">{item.label.slice(0, 1).toUpperCase()}</span><span className="max-w-11 truncate text-center text-[10px] text-[#5f6861]">{item.label.split(" ")[0]}</span></div>)}</div></div>;
}
...
export default function MessagesPage() {
  ...
  const publishStory = trpc.stories.publishText.useMutation({ onSuccess: () => { setStoryComposerOpen(false); setStoryDraft(""); utils.stories.list.invalidate(); toast.success("Story shared for 24 hours"); }, onError: error => toast.error(error.message) });
  ...
  const handlePublishStory = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!storyDraft.trim()) return;
    publishStory.mutate({ textBody: storyDraft.trim(), audience: "private", customAudienceUserIds: [] });
  };

  const newChatDrawer = <Drawer open={newChatOpen} onOpenChange={setNewChatOpen}><DrawerContent className="savanna-new-chat-drawer rounded-t-[28px] border-[#ead2a4] bg-[#fffaf0] dark:border-[#5b4833] dark:bg-[#21180f]"><DrawerHeader className="text-left"><DrawerTitle className="font-display text-2xl text-[#3d2d1a] dark:text-[#fff8ed]">New chat</DrawerTitle><DrawerDescription>Start a private direct or group conversation.</DrawerDescription></DrawerHeader><form className="space-y-3 px-4" onSubmit={handleCreate}><div className="savanna-new-chat-tabs flex rounded-xl bg-[#f5e4bf] p-1"><button type="button" data-active={creationMode === "direct"} onClick={() => setCreationMode("direct")} className={`flex-1 rounded-lg py-2 text-xs font-semibold ${creationMode === "direct" ? "bg-white text-[#7b4a0d] shadow-sm" : "text-[#7b4a0d]"}`}>Chat</button><button type="button" data-active={creationMode === "group"} onClick={() => setCreationMode("group")} className={`flex-1 rounded-lg py-2 text-xs font-semibold ${creationMode === "group" ? "bg-white text-[#7b4a0d] shadow-sm" : "text-[#7b4a0d]"}`}>Group</button></div>{creationMode === "group" ? <Input value={groupTitle} onChange={event => setGroupTitle(event.target.value)} placeholder="Group name" className="savanna-new-chat-input bg-white dark:bg-[#2a2119]" /> : null}<Input value={memberIds} onChange={event => setMemberIds(event.target.value)} placeholder={creationMode === "group" ? "Account IDs, comma-separated" : "Savanna account ID"} aria-label="Savanna account ID" className="savanna-new-chat-input bg-white dark:bg-[#2a2119]" /><DrawerFooter className="px-0"><Button type="submit" disabled={create.isPending} className="savanna-brand-token rounded-xl">{create.isPending ? <Loader2 className="mr-2 size-4 animate-spin" /> : <AnimatedPlusIcon className="mr-2 size-4" />}Create chat</Button></DrawerFooter></form></DrawerContent></Drawer>;
  const storyComposerDrawer = <Drawer open={storyComposerOpen} onOpenChange={setStoryComposerOpen}><DrawerContent className="savanna-new-chat-drawer rounded-t-[28px] border-[#ead2a4] bg-[#fffaf0] dark:border-[#5b4833] dark:bg-[#21180f]"><DrawerHeader className="text-left"><DrawerTitle className="font-display text-2xl text-[#3d2d1a] dark:text-[#fff8ed]">Your Story</DrawerTitle><DrawerDescription>Share a short private Story.</DrawerDescription></DrawerHeader><form className="space-y-3 px-4" onSubmit={handlePublishStory}><Textarea autoFocus value={storyDraft} onChange={event => setStoryDraft(event.target.value)} maxLength={700} placeholder="Share a moment in words" className="savanna-new-chat-input min-h-28 bg-white dark:bg-[#2a2119]" /><DrawerFooter className="px-0"><Button type="submit" disabled={publishStory.isPending || !storyDraft.trim()} className="savanna-brand-token rounded-xl">{publishStory.isPending ? <Loader2 className="mr-2 size-4 animate-spin" /> : <AnimatedPlusIcon className="mr-2 size-4" />}Share Story</Button></DrawerFooter></form></DrawerContent></Drawer>;
  ...
  const chatRows = filteredChatList.map(conversation => <button key={conversation.id} data-active={selectedConversationId === conversation.id} onClick={() => { if (conversation.id < 0) { if (import.meta.env.DEV) { setSelectedConversationId(conversation.id); if (isMobile) setMobileDetail(true); return; } return toast.info("Development preview chat — no real conversation opened"); } setSelectedConversationId(conversation.id); if (isMobile) setMobileDetail(true); }} className="savanna-chat-row flex w-full items-center gap-3 rounded-2xl p-3 text-left transition-colors"><span className="savanna-brand-token grid size-12 shrink-0 place-items-center rounded-full text-sm font-semibold">{conversation.kind === "group" ? <Users className="size-5" /> : "S"}</span><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><span className="truncate text-sm font-semibold text-[#3d2d1a] dark:text-[#fff8ed]">{conversation.title || (conversation.kind === "group" ? "Group chat" : "Private chat")}</span><span className="ml-auto shrink-0 text-[11px] text-[#5f6861] dark:text-[#9AA1A6]">{conversation.id < 0 ? "Preview" : conversation.mutedUntil ? "Muted" : "Active"}</span></span><span className="mt-1 flex items-center gap-1 truncate text-xs text-[#5f6861] dark:text-[#9AA1A6]">{"previewMessage" in conversation ? <>{conversation.previewStatus === "delivered" || conversation.previewStatus === "read" ? <AnimatedCheckCheckIcon size={13} aria-label="Delivered" /> : conversation.previewStatus === "failed" ? <X className="size-3 shrink-0 text-[#FF5B6B]" aria-label="Failed" /> : <Check className="size-3 shrink-0 text-[#AEBAC1]" aria-label="Sent" />}{conversation.previewMessage}</> : conversation.kind === "merchant_support" ? "Merchant support" : "Tap to open your conversation"}</span></span></button>);

  if (isMobile && mobileDetail && selected) return <SavannaShell><div className="savanna-mobile-conversation -mx-4 -mt-5 flex min-h-[calc(100vh-190px)] flex-col bg-[#faf7f0] px-4 dark:bg-[#17120d]"><header className="savanna-mobile-conversation-header flex items-center gap-3 border-b border-[#eadfca] px-4 py-3 dark:border-[#3b2d20]"><Button variant="ghost" size="icon" onClick={() => setMobileDetail(false)} className="rounded-full" aria-label="Back to chats"><ArrowLeft className="size-5" /></Button><span className="savanna-brand-token grid size-10 place-items-center rounded-full">{selected.kind === "group" ? <Users className="size-5" /> : "S"}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-[#3d2d1a] dark:text-[#fff8ed]">{selected.title || (selected.kind === "group" ? "Group chat" : "Private chat")}</p><p className="text-xs text-[#8a765d] dark:text-[#cdb99b]">{selected.kind === "merchant_support" ? "Merchant support" : "Private conversation"}</p></div>{customTabs.length ? <Button variant="ghost" size="icon" onClick={saveSelectedToTab} className="rounded-full" aria-label="Save chat to a tab"><BookmarkPlus className="size-4" /></Button> : null}</header><div className="savanna-mobile-message-thread flex-1 space-y-3 overflow-y-auto p-4">{messages.isLoading ? <Loader2 className="mx-auto mt-10 size-5 animate-spin text-[#9a6410]" /> : messages.data?.length ? messages.data.map(message => <div key={message.id} className={`flex ${message.senderUserId === user?.id ? "justify-end" : "justify-start"}`}><article className={`max-w-[82%] rounded-2xl px-3 py-2.5 text-sm shadow-sm ${message.senderUserId === user?.id ? "bg-[#5d3a0c] text-white" : "savanna-incoming-message bg-white text-[#3d2d1a] dark:text-[#fff8ed]"}`}><p className="whitespace-pre-wrap">{message.contentType === "attachment" ? "Private attachment" : message.payload}</p><p className={`mt-1 flex items-center justify-end gap-1 text-[10px] ${message.senderUserId === user?.id ? "text-[#f8edcf]" : "text-[#5f6861]"}`}>{new Date(message.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}{message.senderUserId === user?.id ? <DeliveryIcon status={message.status} /> : null}</p></article></div>) : <div className="grid h-full place-items-center text-center"><div><MessageCircle className="mx-auto size-8 text-[#d2a34f]" /><p className="mt-3 text-sm font-semibold text-[#5b4934] dark:text-[#f2e7d5]">No messages yet</p></div></div>}</div><form className="savanna-mobile-composer border-t border-[#eadfca] bg-white p-3 dark:border-[#3b2d20] dark:bg-[#21180f]" onSubmit={handleSend}><input ref={attachmentInput} type="file" className="hidden" accept="image/jpeg,image/png,image/webp,application/pdf,audio/mpeg,video/mp4" onChange={handleAttachmentChange} /><div className="savanna-mobile-composer-field flex items-center gap-2 rounded-2xl bg-[#f4f0e8] p-2 dark:bg-[#2a2119]"><Button type="button" variant="ghost" size="icon" onClick={() => attachmentInput.current?.click()} className="size-9 shrink-0 rounded-xl" aria-label="Attach private media"><Paperclip className="size-4" /></Button><Input value={draft} onChange={event => setDraft(event.target.value)} disabled={Boolean(attachment)} placeholder={attachment ? attachment.name : "Write a message"} className="min-w-0 flex-1 border-0 bg-transparent shadow-none focus-visible:ring-0" /><Button type="submit" disabled={(!draft.trim() && !attachment) || send.isPending || sendAttachment.isPending} size="icon" className="savanna-send-button size-9 shrink-0 rounded-xl bg-[#5d3a0c] text-white hover:bg-[#412607]" aria-label="Send message">{send.isPending || sendAttachment.isPending ? <Loader2 className="size-4 animate-spin" /> : <AnimatedSendIcon size={18} pulse={sendPulse} />}</Button></div></form></div></SavannaShell>;

  if (isMobile) return <SavannaShell><div className="savanna-mobile-messages-canvas -mx-4 min-h-[calc(100vh-190px)] bg-white px-4 pb-8 pt-2 dark:bg-[#0A1014]"><label className="savanna-mobile-chat-search mx-2 mt-2 flex h-11 items-center gap-2 rounded-2xl px-4 text-sm dark:text-[#9AA1A6]"><AnimatedSearchIcon size={16} /><input value={conversationSearch} onChange={event => setConversationSearch(event.target.value)} placeholder="Search chats or people" aria-label="Search chats or people" className="min-w-0 flex-1 bg-transparent outline-none placeholder:text-[#a5947e] dark:bg-[#23282C]" /></label><div className="story-rail mt-4 flex gap-2 overflow-x-auto px-3 pb-1" role="tablist" aria-label="Chat filters">{filterTabs.map(([value, label]) => <button key={value} role="tab" aria-selected={chatFilter === value} onClick={() => setChatFilter(value)} className={`shrink-0 rounded-full px-3 py-2 text-xs font-semibold ${chatFilter === value ? "bg-[#e3a43c] text-[#3a260e]" : "bg-[#f4f0e8] text-[#715d43] dark:bg-[#2b2118] dark:text-[#dac7a9]"}`}>{label}</button>)}{customTabs.map(tab => <button key={tab} role="tab" aria-selected={chatFilter === tab} onClick={() => setChatFilter(tab)} className={`shrink-0 rounded-full px-3 py-2 text-xs font-semibold ${chatFilter === tab ? "bg-[#e3a43c] text-[#3a260e]" : "bg-[#f4f0e8] text-[#715d43] dark:bg-[#2b2118] dark:text-[#dac7a9]"}`}>{tab}</button>)}<button onClick={addCustomTab} className="savanna-brand-token grid size-8 shrink-0 place-items-center rounded-full" aria-label="Create a chat tab"><AnimatedPlusIcon size={16} /></button></div><div className="mt-3 divide-y-0 px-0">{conversations.isLoading ? <div className="grid min-h-48 place-items-center"><Loader2 className="size-5 animate-spin text-[#9a6410]" /></div> : filteredChatList.length ? chatRows : <div className="grid min-h-56 place-items-center"><MessageCircle className="size-8 text-[#d2a34f]" /></div>}</div><Button onClick={() => setNewChatOpen(true)} size="icon" className="savanna-brand-token fixed bottom-[calc(5.5rem+env(safe-area-inset-bottom))] right-4 z-40 size-12 rounded-full shadow-none" aria-label="Start a new chat"><AnimatedPlusIcon size={20} /></Button>{newChatDrawer}</div></SavannaShell>;

  return <SavannaShell><div className="savanna-desktop-messages grid min-h-screen lg:grid-cols-[470px_minmax(0,1fr)]"><aside className="savanna-desktop-chat-list flex min-h-0 flex-col border-r p-4"><header className="flex items-center justify-between gap-3"><span className="savanna-wordmark text-[28px]">Savanna</span><Button onClick={() => setNewChatOpen(true)} size="icon" className="savanna-brand-token size-10 rounded-2xl shadow-none" aria-label="Start a new chat"><AnimatedPlusIcon size={19} /></Button></header><label className="savanna-desktop-chat-search mt-5 flex h-11 items-center gap-2 rounded-2xl px-3 text-sm"><AnimatedSearchIcon size={17} /><input value={conversationSearch} onChange={event => setConversationSearch(event.target.value)} placeholder="Search chats or people" aria-label="Search conversations" className="min-w-0 flex-1 bg-transparent outline-none" /></label><DesktopStoryRail items={desktopStoryItems} onCreateStory={() => setStoryComposerOpen(true)} /><div className="savanna-desktop-message-tabs flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Desktop chat filters">{filterTabs.map(([value, label]) => <button key={value} role="tab" aria-selected={chatFilter === value} onClick={() => setChatFilter(value)} data-active={chatFilter === value} className="shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold">{label}</button>)}{customTabs.map(tab => <button key={tab} role="tab" aria-selected={chatFilter === tab} onClick={() => setChatFilter(tab)} data-active={chatFilter === tab} className="shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold">{tab}</button>)}<button type="button" onClick={addCustomTab} className="savanna-brand-token grid size-8 shrink-0 place-items-center rounded-full" aria-label="Create a chat tab"><AnimatedPlusIcon size={15} /></button></div><div className="savanna-desktop-chat-rows mt-3 flex-1 overflow-y-auto">{conversations.isLoading ? <div className="grid min-h-48 place-items-center"><Loader2 className="size-5 animate-spin text-[#A87820]" /></div> : filteredChatList.length ? chatRows : <div className="grid min-h-48 place-items-center"><MessageCircle className="size-7 text-[#9AA1A6]" /></div>}</div></aside><section className="savanna-desktop-conversation-panel flex min-h-0 flex-col">{selected ? <><header className="flex items-center justify-between gap-3 border-b px-6 py-4"><div className="flex items-center gap-3"><span className="savanna-brand-token grid size-10 place-items-center rounded-full">{selected.kind === "group" ? <Users className="size-5" /> : "S"}</span><div><p className="text-sm font-semibold">{selected.title || `${selected.kind === "group" ? "Group" : "Private"} conversation`}</p><p className="mt-0.5 text-xs">{selected.kind === "merchant_support" ? "Merchant support" : "Conversation members only"}</p></div></div><SafetyActions targetDomain="message" targetId={`conversation-${selected.id}`} targetLabel="this conversation" /></header><div className="savanna-desktop-message-thread flex-1 space-y-3 overflow-y-auto p-6">{messages.isLoading ? <Loader2 className="size-5 animate-spin text-[#A87820]" /> : messages.data?.length ? messages.data.map(message => <article key={message.id} className={`group flex ${message.senderUserId === user?.id ? "justify-end" : "justify-start"}`}><div className={`max-w-[78%] rounded-2xl px-4 py-3 shadow-sm ${message.senderUserId === user?.id ? "bg-[#A87820] text-[#111111]" : "savanna-incoming-message"}`}><p className="whitespace-pre-wrap text-sm leading-6">{message.contentType === "attachment" ? "Private attachment" : message.payload}</p>{message.attachments.map(item => <PrivateAttachment key={item.id} attachmentId={item.id} fileName={item.fileName} mimeType={item.mimeType} />)}<div className="mt-1.5 flex items-center justify-end gap-1 text-[10px] opacity-80"><span>{new Date(message.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>{message.senderUserId === user?.id ? <DeliveryIcon status={message.status} /> : null}</div><div className="mt-1 opacity-0 transition-opacity group-hover:opacity-100"><SafetyActions targetDomain="message" targetId={String(message.id)} targetLabel="this message" blockUserId={message.senderUserId} /></div></div></article>) : <div className="grid h-full min-h-72 place-items-center text-center"><div><MessageCircle className="mx-auto size-8 text-[#9AA1A6]" /><p className="mt-4 font-display text-2xl font-semibold">No messages yet</p><p className="mt-2 text-sm">Start the conversation when you are ready.</p></div></div>}</div><form className="savanna-desktop-composer border-t p-4" onSubmit={handleSend}><input ref={attachmentInput} type="file" className="hidden" accept="image/jpeg,image/png,image/webp,application/pdf,audio/mpeg,video/mp4" onChange={handleAttachmentChange} /><div className="flex items-center gap-2 rounded-2xl p-2"><Button type="button" variant="ghost" onClick={() => attachmentInput.current?.click()} size="icon" className="size-10 shrink-0 rounded-xl" aria-label="Attach private media"><Paperclip className="size-4" /></Button><Input value={draft} onChange={event => setDraft(event.target.value)} disabled={Boolean(attachment)} placeholder={attachment ? attachment.name : "Write a message"} aria-label="Message draft" className="min-w-0 flex-1 border-0 bg-transparent shadow-none focus-visible:ring-0" />{attachment ? <Button type="button" variant="ghost" onClick={() => setAttachment(null)} size="icon" className="size-8 shrink-0 rounded-lg" aria-label="Remove attachment"><X className="size-4" /></Button> : null}<Button type="submit" disabled={(!draft.trim() && !attachment) || send.isPending || sendAttachment.isPending} size="icon" className="savanna-send-button size-10 shrink-0 rounded-xl bg-[#D9A441] text-[#111111] hover:bg-[#F2C14E]" aria-label="Send message">{send.isPending || sendAttachment.isPending ? <Loader2 className="size-4 animate-spin" /> : <AnimatedSendIcon size={18} pulse={sendPulse} />}</Button></div></form></> : <div className="grid flex-1 place-items-center p-8 text-center"><div><MessageCircle className="mx-auto size-9 text-[#9AA1A6]" /><p className="mt-4 font-display text-2xl font-semibold">Choose a conversation</p><p className="mt-2 max-w-sm text-sm leading-6">Select an existing conversation from the list, or create a new private chat.</p></div></div>}</section>{newChatDrawer}{storyComposerDrawer}</div></SavannaShell>;
}
