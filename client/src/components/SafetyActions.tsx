...
export function SafetyActions({ targetDomain, targetId, targetLabel, blockUserId }: SafetyActionsProps) {
  ...
  if (!open) {
    return <Button variant="ghost" size="sm" onClick={() => setOpen(true)} className="h-8 rounded-lg px-2 text-xs text-[#5F6861] hover:bg-[#D9A441]/10 hover:text-[#D9A441]">Safety</Button>;
  }
  ...
  return (
    <aside aria-label={`Safety actions for ${targetLabel}`} className="rounded-2xl border border-[#ead9bc] bg-[#fffaf0] p-4 shadow-[0_14px_30px_rgba(88,64,29,0.08)]">
      ...
      <form className="mt-4 space-y-3" onSubmit={async event => {
        ...
        try {
          await createFirebaseSafetyReport({ reporter: user, targetDomain, targetId, reason, evidenceScope, detail: detail.trim() || undefined });
        ...
        <div className="flex flex-wrap gap-2"><Button type="submit" disabled={reportPending} className="savanna-brand-token h-9 rounded-lg text-xs shadow-none"><Flag className="mr-1.5 size-3.5" />Send report</Button>{firebaseBlockUserId ? <Button type="button" variant="outline" disabled={blockPending} onClick={async () => {
        ...
        }} className="h-9 rounded-lg border-[#dfbfb5] bg-white text-xs text-[#87452f] hover:bg-[#fff1eb]"><ShieldAlert className="mr-1.5 size-3.5" />Block account</Button> : null}</div>
      </form>
    </aside>
  );
}
