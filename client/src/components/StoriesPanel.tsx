...
export function MobileStoriesHeader({ onOpenProfile }: { onOpenProfile: () => void }) {
  ...
  return <><header className="savanna-mobile-header fixed inset-x-0 top-0 z-40 bg-[#f7f6f1]/92 backdrop-blur-xl dark:bg-[#0A1014]/95 lg:hidden"><div className="flex h-[68px] items-center justify-between gap-3 px-4"><div className="flex min-w-0 items-center gap-2"><Button variant="ghost" size="icon" onPointerDown={() => setMenuPulse(current => current + 1)} onClick={onOpenProfile} aria-label="Open profile" className="size-14 shrink-0 rounded-xl text-[#3d2d1a] dark:text-[#fff8ed]"><AnimatedMenuIcon className="size-6" size={24} pulse={menuPulse} /></Button>{collapsedStoriesCluster}<Link href="/" aria-label="Savanna messages" className="flex shrink-0 items-center whitespace-nowrap text-2xl leading-none"><span className="savanna-wordmark">Savanna</span></Link></div><div className="flex shrink-0 items-center"><Button variant="ghost" size="icon" onPointerDown={() => setSearchPulse(current => current + 1)} aria-label="Search Savanna" className="size-14 rounded-full"><AnimatedSearchIcon className="size-6" size={24} pulse={searchPulse} /></Button></div></div><section
    aria-label="Stories"
